...
export const getRequiredParamPerNetwork = <T>(poolConfig: PoolConfiguration, key: keyof PoolConfiguration, network: eNetwork): T => {
...
export const getAddressFromConfig = (param: iParamsPerNetwork<string | undefined>, network: eNetwork, key?: string): tEthereumAddress => {
...
export const savePoolTokens = async (reservesConfig: ITokenAddress, dataProviderAddress: tEthereumAddress) => {
    const dataProviderArtifact = await hre.deployments.get(POOL_DATA_PROVIDER);
    const dataProvider = (await hre.ethers.getContractAt(dataProviderArtifact.abi, dataProviderAddress)) as AaveProtocolDataProvider;
...
export const getReserveAddresses = async (poolConfig: IBaseConfiguration, network: eNetwork) => {
    ...
    const testnetTokenKeys = Object.keys(allDeployments).filter((key) => key.includes(TESTNET_TOKEN_PREFIX) && reservesKeys.includes(key.replace(TESTNET_TOKEN_PREFIX, "")));
...
export const getChainlinkOracles = async (poolConfig: IBaseConfiguration, network: eNetwork) => {
    ...
    const testnetKeys = Object.keys(allDeployments).filter(
        (key) =>
            ...
            (reservesKeys.includes(key.replace(TESTNET_PRICE_AGGR_PREFIX, "")) || rewardKeys.includes(key.replace(TESTNET_PRICE_AGGR_PREFIX, "")))
    );
...
export const getTreasuryAddress = async (poolConfig: IBaseConfiguration, network: eNetwork): Promise<tEthereumAddress> => {
    const treasuryConfigAddress = getParamPerNetwork<string>(poolConfig.ReserveFactorTreasuryAddress, network);
...
export const isTestnetMarket = (poolConfig: ICommonConfiguration): boolean => !isProductionMarket(poolConfig);
